...
export default function Gallery({ images }) {
  ...
  const [activeImage, updateImage] = useState({
    url: images[0],
    currentIndex: 0,
  });
  ...
  function toggleImage(direction) {
    if (direction === "right") {
      if (amountOfImages - 1 > activeImage.currentIndex) {
        updateImage({
          url: images[activeImage.currentIndex + 1],
          currentIndex: activeImage.currentIndex + 1,
        });
      } else {
  ...
  return (
    <header className="gallery">
      ...
      <span className="photo-index">
        {activeImage.currentIndex + 1}/{amountOfImages}
      </span>
    </header>
  );
}
